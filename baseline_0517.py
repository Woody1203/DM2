...
def baselineTest():
    ...
    pred = baseline(trainning_data, testing_data)
    #B = B.drop(['mean'])
    #         .replace.iloc[i-1][j-1] = B.loc['mean'][j-1]

    elapsed_time = time.time() - start_time
    print("********************************** the code finished in : " + str(elapsed_time/60) + " minutes*********")
    # print(B.shape)
    # print(pred.shape)
    print("mse",mse(pred, testing_data))
    print("mae",mae(pred, testing_data))
